...
import { ADMIN_PRODUCTS_COLOR_LIST, ADMIN_PRODUCTS_COLOR_STATUS } from "apiurl";
...
export default function ChangePassword() {
    ...
    const handleChangePage = (_, newPage) => {
        setPage(newPage);
        getProductList(newPage * rowsPerPage, rowsPerPage);
    ...
    const getProductList = async (page, rowsPerPage) => {
        ...
        const response = await axiosRequest(ADMIN_PRODUCTS_COLOR_LIST, data, config);
    ...
    const statusChange = async (id, status) => {
        ...
        const response = await axiosRequest(ADMIN_PRODUCTS_COLOR_STATUS, data, config);
        if (response.data.status === true) {
            getProductList(page, rowsPerPage);
        }
    }
    ...
    return (
        <Container>
            ...
            <Box className="breadcrumb">
                <Breadcrumb routeSegments={[{ name: "Products Color", path: "/products-color-listing" }, { name: "Home" }]} />
            </Box>
            <SimpleCard title="Product Color Details">
                <Button variant="contained" size={"small"} style={{ float: 'right' }} color="primary" onClick={() => { navigate('/products-color-create') }}>Add New Record</Button>
                                            ...
                                            <TableRow key={index}>
                                                ...
                                                <TableCell align="right" style={{ float: 'right !importent' }}>
                                                    <Grid container spacing={1} alignItems="right">
                                                        <Grid item>
                                                            <EditButton label={'Edit'} onClick={() => { navigate(`/products-color-edit/${subscriber.id}`) }} />
                                                        </Grid>
                                                    </Grid>
                                                </TableCell>
